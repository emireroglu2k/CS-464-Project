...
Runs exhaustive Random Forest experiments (30+ configurations).
...
from data_loader import Model1DataLoader  # noqa: E402
...
EXPERIMENTS = [
    ...
    # Hypothesis: More trees = better stability, but diminishing returns after ~100.
    ...
    # Hypothesis: Shallow trees underfit (low acc), Deep trees overfit (high train acc, low test acc).
    ...
    # Hypothesis: 'log2' looks at fewer features per split than 'sqrt', making trees more random/uncorrelated.
    ...
    {"name": "feat_0.5",          "n_estimators": 100, "max_depth": None, "min_samples_split": 2,  "min_samples_leaf": 1, "max_features": 0.5,    "criterion": "gini", "bootstrap": True}, # Looks at 50% of features (Expensive!)
]

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run exhaustive Random Forest experiments.")
    p.add_argument("--dataset-root", type=str, default=str(THIS_DIR.parents[1] / "dataset"))
...
def main() -> int:
    ...
    print(f"\nSTARTING EXHAUSTIVE EXPERIMENTS ({len(EXPERIMENTS)} configs)")
    print("=" * 100)

    # --- 1. LOAD DATA & CACHE FEATURES ---
    data_loader = Model1DataLoader(dataset_root=dataset_root, image_size=args.image_size, batch_size=args.batch_size)
    train_ds, valid_ds, test_ds, class_names = data_loader.load_splits()
    ...
    lines.append("=" * 160)
    lines.append(f"MASTER EXPERIMENT SUMMARY | {rf_utils.utc_now_iso()}")
    lines.append("=" * 160)
