"""
train_experiments.py - Model 2: Custom Preprocessed Train with Augmentation
Runs exhaustive Random Forest experiments.
...
from data_loader import Model2DataLoader  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run exhaustive Random Forest experiments for Model 2.")
    p.add_argument("--preprocessed-train", type=str, default=str(THIS_DIR / "preprocessed_train"))
...
def main() -> int:
    ...
    preprocessed_train = Path(args.preprocessed_train)
    ...
    print(f"\nMODEL 2: CUSTOM PREPROCESSED TRAIN")
    ...
    data_loader = Model2DataLoader(
        preprocessed_train_root=preprocessed_train,
    ...
    print(f"Valid samples: {len(valid_ds)} (from original dataset)")
    print(f"Test samples:  {len(test_ds)} (from original dataset)")
    ...
    for i, config in enumerate(EXPERIMENTS, 1):
        ...
        outcome_lines = [
            ...
            f"model: model2-custom-preprocess",
    ...
    lines.append("=" * 110)
    lines.append(f"MASTER EXPERIMENT SUMMARY - MODEL 2 (Custom Preprocessed Train) | {rf_utils.utc_now_iso()}")
    lines.append("=" * 110)
