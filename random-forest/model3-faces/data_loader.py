"""Data loader for Model 3: Processed faces dataset with ImageFolder structure."""
...
class Model3DataLoader:
    """
    Loads data from processed-dataset-faces (ImageFolder structure).
    Train/valid/test splits with breed subfolders.
    """
    
    def __init__(
        self,
        processed_faces_root: Path,
        image_size: int = 224,
        batch_size: int = 32,
    ):
        self.processed_faces_root = processed_faces_root
    ...
    def verify_paths_exist(self) -> None:
        """Check if processed faces directory exists."""
        if not self.processed_faces_root.exists():
            raise FileNotFoundError(
                f"Processed faces dataset not found: {self.processed_faces_root}\n"
                f"Run preprocess.py first to generate processed face images."
            )
    ...
    def load_splits(self, augmentation_strategy: str = "all") -> Tuple[datasets.ImageFolder, datasets.ImageFolder, datasets.ImageFolder, List[str]]:
        ...
        train_dir = self.processed_faces_root / "preprocessed_train"
        valid_dir = self.processed_faces_root / "valid"
        test_dir = self.processed_faces_root / "test"
