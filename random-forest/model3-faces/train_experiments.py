"""
train_experiments.py - Model 3: Face-Cropped Dataset
Runs exhaustive Random Forest experiments on cropped cat faces.
...
from data_loader import Model3DataLoader  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run exhaustive Random Forest experiments for Model 3.")
    p.add_argument("--processed-faces", type=str, default=str(THIS_DIR / "processed-dataset-faces"))
...
def main() -> int:
    ...
    processed_faces = Path(args.processed_faces)
    ...
    data_loader = Model3DataLoader(
        processed_faces_root=processed_faces,
    ...
    print(f"Valid samples: {len(valid_ds)} (face-cropped)")
    print(f"Test samples:  {len(test_ds)} (face-cropped)")
    ...
    lines.append("=" * 110)
    lines.append(f"MASTER EXPERIMENT SUMMARY - MODEL 3 (Face-Cropped Dataset) | {rf_utils.utc_now_iso()}")
    lines.append("=" * 110)
